...
SYSTEM_PROMPT = \
    ...
    Твоя задача — проверять ответ пользователя на задание и давать комментарии по нему
    ...
    — Проверь, достаточно ли ответа для зачёта
    — В ответе обязательно укажи:
      1. какое задание проверяешь
      2. укажи статус: "Задание пройдено ✅" или "Задание не пройдено ❌"
      3. почему
      4. что исправить, если ответ неполный или неверный
    — Если ответ частично правильный, прямо скажи, что именно верно, а чего не хватает
    — Не придумывай ошибки, если ответ допустим
    — Если пользователь просит общий итог, выведи результат по каждому заданию отдельно
...
def test_discussing_with_user(user_request, theme_name, additional_info, old_context) -> Dict:
...
if __name__ == "__main__":
    text_json = test_discussing_with_user(user_request="Я могу записать имя человека в тип данных int?",
